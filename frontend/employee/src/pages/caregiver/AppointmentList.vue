...
import { caregiverApi } from '@/api/caregiverApi.js';
    ...
    const createAppointment = async () => {
      ...
      if (caregivers.value.length === 0) {
        ...
        try {
          ...
          console.log('✅ 照服員列表載入完成:', caregivers.value);
        } catch (error) {
          console.error('❌ 載入照服員列表失敗:', error);
          showToast({
            title: '載入失敗',
            message: '無法載入照服員列表，請稍後再試',
            type: 'error'
          });
          return;
        }
...
const loadCaregivers = async () => {
  try {
    ...
    // 📡 呼叫 API 取得照服員列表 - 使用 caregiverApi 而不是 scheduleApi
...
const handleCreateAppointment = async (appointmentData) => {
  ...
  try {
    ...
    showToast({
      title: '建立失敗',
      message: error.response?.data?.message || error.message || '建立虛擬訂單失敗，請稍後再試',
      type: 'error'
    });
    throw error; // 重新拋出錯誤，讓子元件也能處理
  }
};
...
const handleCheckConflict = async (conflictData, resolve) => {
  try {
    ...
    onMounted(async () => {
      ...
      // 🔧 預先載入照服員列表
      try {
        console.log('🔄 預先載入照服員列表...');
        await loadCaregivers();
        console.log('✅ 照服員列表預載入完成:', caregivers.value.length, '位');
      } catch (error) {
        console.error('❌ 預載入照服員列表失敗:', error);
        // 不顯示錯誤通知，因為這只是預載入
      }
    });
