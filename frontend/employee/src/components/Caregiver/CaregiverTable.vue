<template>
  <div class="card h-100 p-0 radius-12">
    ...
    <div class="card-body p-24">
      ...
      <!-- 沒有資料時的提示 -->
      <div
        v-else-if="filteredCareWorkers.length === 0"
      ...
      <div v-else class="table-responsive scroll-sm">
        <table class="table bordered-table sm-table mb-0">
          ...
          <tbody>
            <tr
              v-for="(careWorker, index) in filteredCareWorkers"
              :key="careWorker.id"
            >
              <td>
                <div class="d-flex align-items-center gap-10">
              ...
              <td>
                <span
                  class="px-12 py-4 radius-4 fw-medium text-sm bg-primary-50 text-primary-600 border border-primary-200"
                >
      ...
      <!-- 分頁元件 -->
      <Pagination
        v-if="filteredCareWorkers.length > 0"
...
import { useConfirmDialog } from "@/composables/useConfirmDialog"; // 引入確認對話框
...
const { showConfirmDialog } = useConfirmDialog(); // 使用確認對話框
...
const endIndex = computed(() => {
  return Math.min(
    startIndex.value + entriesPerPage.value - 1,
    totalEntries.value
  );
});
...
const deleteCareWorker = async (careWorker) => {
  // **重點註解：改用 useConfirmDialog 取代原生 confirm，提供更好的使用者體驗**
  try {
    const confirmed = await showConfirmDialog({
      title: "刪除照服員",
      message: `確定要刪除照服員「${careWorker.chineseName}」的資料嗎？\n\n此操作無法復原。`,
      type: "warning",
      confirmText: "確定刪除",
      cancelText: "取消",
    ...
    if (confirmed) {
      try {
        ...
        showToast({
          title: "刪除成功",
          message: `照服員「${careWorker.chineseName}」已成功刪除`,
          type: "success",
        });
      } catch (deleteErr) {
