...
public interface CaregiverAppointmentDao {
    ...
    // 檢查時間衝突
    List<CaregiverAppointment> checkTimeConflict(Integer caregiverId, LocalDateTime startTime, LocalDateTime endTime, Integer excludeId);

    // 取得照服員特定時間範圍的預約
    List<CaregiverAppointment> getCaregiverAppointments(Integer caregiverId, LocalDateTime startDate, LocalDateTime endDate);
