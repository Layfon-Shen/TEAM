...
public class ResDAOImpl implements ReservationDAO {
	...
	public Integer insertRes(ResCreateRequest resCreateRequest, LocalDateTime createdAt, LocalDateTime scheduledAt,
		...
		final String sql = "INSERT INTO Reservation (member_id, bus_id, "
	...
	// 修改預約表單
	@Override
	public Reservation updateRes(ResRequest resRequest, Integer distanceMeters, Integer price) {
		...
		final String sql1 = "SELECT * FROM Reservation WHERE id = :id";
	...
	public Reservation findById(int id) {
		final String sql = "SELECT * FROM Reservation WHERE id = :id";
		Map<String, Object> fiRes = new HashMap<>();
		fiRes.put("id", id);

		List<Reservation> resList = namedParameterJdbcTemplate.query(sql, fiRes, new ReservationRowMapper());
		if (!resList.isEmpty()) {
	...
	@Override
	public List<Reservation> findByFilter(Integer memberId, String startAddress, String endAddress, LocalDateTime from,
			LocalDateTime to) {
	...
	public int markCompleted(int id, LocalDateTime completedAt) {
		final String sql = "UPDATE Reservation SET completed_at = :completedAt, "
